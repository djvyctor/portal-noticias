<template>
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold text-gray-800 mb-6">Minhas Notícias Rejeitadas</h1>
    ...
    <div v-else-if="news.data && news.data.length === 0" class="text-center py-12">
      <p class="text-gray-500">Você não tem notícias rejeitadas.</p>
    </div>
    ...
    <div v-else class="space-y-6">
      <div
        ...
        :key="item.id"
        class="bg-white shadow-md rounded-lg overflow-hidden border-l-4 border-red-500"
      >
        <div class="p-6">
          ...
          <div class="mb-4 bg-red-50 border-l-4 border-red-400 p-4">
            <p class="text-red-800 font-semibold mb-1">Motivo da Rejeição:</p>
            <p class="text-red-700">{{ item.rejection_reason || 'Não informado' }}</p>
          </div>
    ...
    <div v-if="showViewModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50" @click.self="showViewModal = false">
      <div class="relative top-10 mx-auto p-5 border w-full max-w-4xl shadow-lg rounded-md bg-white my-8">
        <div class="mt-3">
          ...
          <div class="mb-4 bg-red-50 border-l-4 border-red-400 p-4">
            <p class="text-red-800 font-semibold mb-1">Motivo da Rejeição:</p>
            <p class="text-red-700">{{ viewingNews?.rejection_reason || 'Não informado' }}</p>
          </div>
...
const loadNews = async (page = 1) => {
  ...
  error.value = null
  try {
    const response = await newsAPI.rejected()
